...
function loadStudentStats() {
    ...
    if (student) {
        ...
        document.querySelector('.stat-card:nth-child(1) p').textContent = `Class ${student.class}`;
...
function updateAttendanceHistory(records) {
    ...
    if (records.length === 0) {
        const row = document.createElement('tr');
        row.innerHTML = '<td colspan="5" class="no-data">No attendance records available</td>';
        tableBody.appendChild(row);
        return;
    }
...
function loadStudentAcademicResults() {
    ...
    updateMarksHistory(studentMarks, marksData);
}
...
function updateMarksHistory(studentMarks, allMarks) {
    const tableBody = document.getElementById('marksHistoryTable');
    tableBody.innerHTML = '';
    
    if (studentMarks.length === 0) {
        const row = document.createElement('tr');
        row.innerHTML = '<td colspan="5" class="no-data">No academic results available</td>';
        tableBody.appendChild(row);
        return;
    }
    ...
    studentMarks.forEach(mark => {
        const key = `${mark.class}-${mark.section}-${mark.subject}`;
        if (!classAverages[key]) {
            // Calculate class average for this subject
            const classSubjectMarks = allMarks.filter(
                record => record.class === mark.class && 
                          record.section === mark.section && 
                          record.subject === mark.subject
            );
            
            const total = classSubjectMarks.reduce((sum, record) => sum + parseFloat(record.marks), 0);
            classAverages[key] = classSubjectMarks.length ? (total / classSubjectMarks.length).toFixed(1) : 'N/A';
        }
    });
    ...
    studentMarks.forEach(mark => {
        ...
        row.className = getGradeClass(grade);
        
        row.innerHTML = `
            <td>${mark.subject}</td>
            <td>${mark.marks}</td>
            <td>${grade}</td>
            <td>${classAverages[key]}</td>
